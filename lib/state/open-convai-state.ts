...
export class OpenConvaiState implements IStateManager {
   ...
   */
  /**
   * Converts a string status to SDK connection status format
   * @param status - Connection status as string
   * @returns Normalized status format for SDK
   */
  ...
  private convertToActiveConnection(conn: Connection): ActiveConnection {
    return {
      ...
      targetAgentId: conn.targetAccountId,
    };
  }

  /**
   * Converts SDK status to state status format
   * @param status - Connection status from SDK
   * @returns Normalized ConnectionStatus for state management
   */
