...
export class HCS10Client {
  ...
  public async createAndRegisterAgent(
    ...
    console.log('Creating agent with metadata:', JSON.stringify(metadata, null, 2));
    console.log('Network:', this.getNetwork());
    console.log('Operator ID:', this.getOperatorId());
    ...
    console.log('Agent builder configured');
    ...
    if (metadata?.feeConfig) {
      ...
      console.log('Fee configuration applied');
    }
    ...
    try {
      ...
      console.log('Calling SDK with initialBalance:', hasFees ? 50 : undefined);
      ...
      if (
        ...
        console.log('Agent channels set successfully');
      } else {
        console.log('Warning: Missing topic IDs in result:', result);
      }
      return result;
    } catch (error) {
